...
function dumprow(row) {
  ...
  rowStr += ",バイエル符号=" + row.CON_SHORT + " " + row.CODE;
  rowStr += ",等級=" + row.VMAG;
...
function butotnClick(){
  let sql = "SELECT * FROM ORIGIN INNER JOIN APPENDIX ON ORIGIN.IAU_NAME = APPENDIX.IAU_NAME INNER JOIN CONS ON ORIGIN.CON_SHORT = CONS.CON_SHORT WHERE CONS.CON_JA ='" + keyText.value + "\';";
  execSqljs(sql);
}

/* 全検索 */
function buttonAllClick() {
  let sql = "SELECT * FROM ORIGIN INNER JOIN APPENDIX ON ORIGIN.IAU_NAME = APPENDIX.IAU_NAME INNER JOIN CONS ON ORIGIN.CON_SHORT = CONS.CON_SHORT;";
  execSqljs(sql);
}
